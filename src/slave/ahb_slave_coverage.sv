...
class ahb_slave_coverage extends uvm_subscriber #(ahb_slave_tx);
    `uvm_component_utils(ahb_slave_coverage)

    covergroup ahb_slave_covergroup with function sample(ahb_slave_tx packet);
...
function void ahb_slave_coverage::report_phase(uvm_phase phase);
  `uvm_info(get_type_name(),$sformatf("AHB slave Agent Coverage = %0.2f %%", ahb_slave_covergroup.get_coverage()), UVM_NONE);
